...
# 用Scroll获取所有数据
def scroll(host, index, data, size=100, timeout="10m"):
    first = True  # 是否是首次查询
    data["size"] = size
    data = json.dumps(data)  # encode
    ...
    while True:
        try:
            ...
            logger.error(e)
            logger.error("url:{0}".format(host + "_search/scroll"))
            logger.error("scroll_data:{0}".format(scroll_data))
            logger.error("index:{0}".format(index))
