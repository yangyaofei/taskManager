...
#  字典 key word		value TFIDF_word TFIDF_frq TFIDF_sum_frq 的字典
...
def updateWords(words):
	...
	query = TFIDF_word.update(TFIDF_frq=case_frq, TFIDF_sum_frq=case_sum_frq)\
		.where(TFIDF_word.TFIDF_word << w)
	# query_sum_frq =  TFIDF_word.update(TFIDF_sun_frq = case_sum_frq)\
